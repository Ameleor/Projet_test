...
def layout(node) :
    if node.is_leaf() :
        ...
        #modified_name = ' '.join(parts[-3:]) -> trop simpliste et certaines exceptions ne sont pas prises en compte
        #modified_name = '_'.join(parts[:2]) + " " + ' '.join(parts[-3:]) -> ne fonctionne pas car certaines exceptions ne sont pas prises en compte
        modified_name ='_'.join(parts[-1].split('_')[:2])
...
t1= Tree("results/tree/PGC_crtI/PGC_crtI.bionj")
t2= Tree("results/tree/PGC_crtE/PGC_crtE.bionj")
t3= Tree("results/tree/PGC_crtD/PGC_crtD.bionj")
